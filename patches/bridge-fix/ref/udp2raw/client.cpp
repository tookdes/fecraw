...
int client_on_raw_recv_hs2_or_ready(conn_info_t &conn_info, char type, char *data, int data_len) {
    ...
    if (data_len >= 0 && type == 'h') {
        ...
        if (bridge_mode) {
            int ret = send(udp_fd, data + sizeof(u32_t), data_len - (sizeof(u32_t)), 0);
            if (ret < 0) {
                mylog(log_warn, "bridge send returned %d,%s\n", ret, get_sock_error());
            }
        } else {
...
int client_on_udp_recv(conn_info_t &conn_info) {
    ...
    if (bridge_mode) {
        recv_len = recv(udp_fd, buf, max_data_len + 1, 0);
        if (recv_len < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) return 0;
            mylog(log_debug, "bridge recv error,%s\n", get_sock_error());
            return -1;
        }
    } else {
    ...
    if (bridge_mode) {
        static u32_t bridge_conv = 0;
        static int bridge_conv_init = 0;
        if (!bridge_conv_init) {
            bridge_conv = conn_info.blob->conv_manager.c.get_new_conv();
            address_t dummy;
            memset(&dummy, 0, sizeof(dummy));
            dummy.inner.ipv4.sin_family = AF_INET;
            dummy.inner.ipv4.sin_addr.s_addr = htonl(0x7f000001);
            dummy.inner.ipv4.sin_port = htons(1);
            conn_info.blob->conv_manager.c.insert_conv(bridge_conv, dummy);
            bridge_conv_init = 1;
            mylog(log_info, "bridge conv created, conv_id=%x\n", bridge_conv);
        }
        conv = bridge_conv;
    } else {
        address_t tmp_addr;
