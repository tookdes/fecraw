...
static fd64_t g_bridge_fd64 = 0;
static conn_info_t *g_bridge_conn = NULL;
static u32_t g_bridge_conv = 0;
static int g_bridge_conv_valid = 0;
...
// receives data and heart beat by recv_safer.
{
    ...
    if (type == 'h' && data_len >= 0) {
        ...
        if (!conn_info.blob->conv_manager.s.is_conv_used(tmp_conv_id)) {
            ...
            if (bridge_mode) {
                conn_info.blob->conv_manager.s.insert_conv(tmp_conv_id, g_bridge_fd64);
                if (!g_bridge_conv_valid) {
                    g_bridge_conv = tmp_conv_id;
                    g_bridge_conv_valid = 1;
                }
                mylog(log_info, "[%s]bridge conv_id=%x mapped to bridge fd\n", ip_port, tmp_conv_id);
            } else {
...
// connection recovery is also handle here
{
    ...
    if (conn_manager.const_id_mp.find(tmp_oppsite_const_id) == conn_manager.const_id_mp.end()) {
        ...
        if (bridge_mode && g_bridge_fd64 != 0) {
            g_bridge_conn = &conn_info;
            fd_manager.get_info(g_bridge_fd64).p_conn_info = &conn_info;
            g_bridge_conv_valid = 0;
            mylog(log_info, "[%s]bridge: conn associated with bridge fd\n", ip_port);
        }
        ...
        if (ori_conn_info.state.server_current_state == server_ready) {
            ...
            if (bridge_mode && g_bridge_fd64 != 0) {
                g_bridge_conn = &ori_conn_info;
                fd_manager.get_info(g_bridge_fd64).p_conn_info = &ori_conn_info;
            }
...
int server_event_loop() {
    ...
    if (bridge_mode) {
        ...
        g_bridge_fd64 = fd_manager.create(udp_fd);
        fd_manager.get_info(g_bridge_fd64).p_conn_info = NULL;

        ev.events = EPOLLIN;
        ev.data.u64 = g_bridge_fd64;
        ret = epoll_ctl(epollfd, EPOLL_CTL_ADD, udp_fd, &ev);
        if (ret != 0) {
            mylog(log_fatal, "add bridge_fd to epoll error\n");
            myexit(-1);
        }
        mylog(log_info, "bridge fd registered with epoll, fd64=%lld\n", (long long)g_bridge_fd64);
    }
    ...
    while (1)  ////////////////////////
    {
        ...
        for (idx = 0; idx < nfds; ++idx) {
            ...
            if ((events[idx].data.u64) == (u64_t)timer_fd) {
                ...
                if (bridge_mode && fd64 == g_bridge_fd64) {
                    if (p_conn_info == NULL || !g_bridge_conv_valid) {
                        char tmp[buf_len];
                        recv(fd_manager.to_fd(fd64), tmp, sizeof(tmp), 0);
                        continue;
                    }
                    conn_info_t &bconn = *p_conn_info;
                    char bbuf[buf_len];
                    int blen = recv(fd_manager.to_fd(fd64), bbuf, max_data_len + 1, 0);
                    if (blen > 0 && blen <= max_data_len) {
                        if (bconn.state.server_current_state == server_ready) {
                            send_data_safer(bconn, bbuf, blen, g_bridge_conv);
                        }
                    }
                    continue;
                }
