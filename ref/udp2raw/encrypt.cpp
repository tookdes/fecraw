...
#define FECRAW_HAS_OPENSSL 1
#endif
#endif

#ifndef FECRAW_HAS_OPENSSL
#if defined(FECRAW_USE_OPENSSL) || defined(USE_OPENSSL)
#define FECRAW_HAS_OPENSSL 1
#endif
#endif

#ifdef FECRAW_HAS_OPENSSL
...
unordered_map<int, const char *> cipher_mode_tostring = {
    ...
    {cipher_chacha20poly1305, "chacha20poly1305"},
};
...
int my_encrypt(const char *data, char *output, int &len /*,char * key*/) {
    ...
    if (cipher_is_aead(cipher_mode))
        return encrypt_AEAD((uint8_t *)data, (uint8_t *)output, len, NULL, NULL, 0);
...
int my_decrypt(const char *data, char *output, int &len /*,char * key*/) {
    ...
    if (cipher_is_aead(cipher_mode))
        return decrypt_AEAD((uint8_t *)data, (uint8_t *)output, len, NULL, NULL, 0);
...
/*
 * AEAD encrypt/decrypt using OpenSSL EVP.
 * Wire format: [12B nonce][ciphertext][16B tag]
 * auth_mode is forced to auth_none when AEAD is active.
 */

static const int AEAD_NONCE_LEN = 12;
static const int AEAD_TAG_LEN   = 16;

#ifdef FECRAW_HAS_OPENSSL

static int aead_encrypt_openssl(const char *data, char *output, int &len,
                                unsigned char *key, int key_len, const EVP_CIPHER *evp) {
    unsigned char nonce[AEAD_NONCE_LEN];
    if (RAND_bytes(nonce, AEAD_NONCE_LEN) != 1) return -1;
    ...
    if (EVP_EncryptInit_ex(ctx, evp, NULL, NULL, NULL) != 1) goto cleanup;
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, AEAD_NONCE_LEN, NULL) != 1) goto cleanup;
    if (EVP_EncryptInit_ex(ctx, NULL, NULL, key, nonce) != 1) goto cleanup;
    if (EVP_EncryptUpdate(ctx, (unsigned char *)output + AEAD_NONCE_LEN, &outl,
                          (const unsigned char *)data, len) != 1) goto cleanup;
    if (EVP_EncryptFinal_ex(ctx, (unsigned char *)output + AEAD_NONCE_LEN + outl, &final_len) != 1) goto cleanup;
    outl += final_len;

    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, AEAD_TAG_LEN,
                            output + AEAD_NONCE_LEN + outl) != 1) goto cleanup;

    memcpy(output, nonce, AEAD_NONCE_LEN);
    len = AEAD_NONCE_LEN + outl + AEAD_TAG_LEN;
    ret = 0;
...
static int aead_decrypt_openssl(const char *data, char *output, int &len,
                                unsigned char *key, int key_len, const EVP_CIPHER *evp) {
    if (len < AEAD_NONCE_LEN + AEAD_TAG_LEN) return -1;

    const unsigned char *nonce = (const unsigned char *)data;
    int ciphertext_len = len - AEAD_NONCE_LEN - AEAD_TAG_LEN;
    ...
    if (EVP_DecryptInit_ex(ctx, evp, NULL, NULL, NULL) != 1) goto cleanup;
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, AEAD_NONCE_LEN, NULL) != 1) goto cleanup;
    if (EVP_DecryptInit_ex(ctx, NULL, NULL, key, nonce) != 1) goto cleanup;
    if (EVP_DecryptUpdate(ctx, (unsigned char *)output, &outl,
                          (const unsigned char *)data + AEAD_NONCE_LEN, ciphertext_len) != 1) goto cleanup;

    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, AEAD_TAG_LEN,
                            (void *)(data + AEAD_NONCE_LEN + ciphertext_len)) != 1) goto cleanup;

    if (EVP_DecryptFinal_ex(ctx, (unsigned char *)output + outl, &final_len) != 1) goto cleanup;
    outl += final_len;
...
static int cipher_aes256gcm_encrypt(const char *data, char *output, int &len) {
    return aead_encrypt_openssl(data, output, len, cipher_key_encrypt, 32, EVP_aes_256_gcm());
}

static int cipher_aes256gcm_decrypt(const char *data, char *output, int &len) {
    return aead_decrypt_openssl(data, output, len, cipher_key_decrypt, 32, EVP_aes_256_gcm());
}

static int cipher_chacha20poly1305_encrypt(const char *data, char *output, int &len) {
    return aead_encrypt_openssl(data, output, len, cipher_key_encrypt, 32, EVP_chacha20_poly1305());
}

static int cipher_chacha20poly1305_decrypt(const char *data, char *output, int &len) {
    return aead_decrypt_openssl(data, output, len, cipher_key_decrypt, 32, EVP_chacha20_poly1305());
}

#else /* !FECRAW_HAS_OPENSSL */

static int cipher_aes256gcm_encrypt(const char *, char *, int &) {
    mylog(log_fatal, "AES-256-GCM requires OpenSSL; rebuild with -DFECRAW_USE_OPENSSL -lssl -lcrypto\n");
    return -1;
}
static int cipher_aes256gcm_decrypt(const char *, char *, int &) { return -1; }
static int cipher_chacha20poly1305_encrypt(const char *, char *, int &) {
    mylog(log_fatal, "ChaCha20-Poly1305 requires OpenSSL; rebuild with -DFECRAW_USE_OPENSSL -lssl -lcrypto\n");
    return -1;
}
static int cipher_chacha20poly1305_decrypt(const char *, char *, int &) { return -1; }

#endif /* FECRAW_HAS_OPENSSL */

int encrypt_AEAD(uint8_t *data, uint8_t *output, int &len, uint8_t *key, uint8_t *header, int hlen) {
    (void)key; (void)header; (void)hlen;
    switch (cipher_mode) {
        case cipher_aes256gcm:
            return cipher_aes256gcm_encrypt((const char *)data, (char *)output, len);
        case cipher_chacha20poly1305:
            return cipher_chacha20poly1305_encrypt((const char *)data, (char *)output, len);
        default:
            return -1;
    }
}
...
int decrypt_AEAD(uint8_t *data, uint8_t *output, int &len, uint8_t *key, uint8_t *header, int hlen) {
    ...
    switch (cipher_mode) {
        case cipher_aes256gcm:
            return cipher_aes256gcm_decrypt((const char *)data, (char *)output, len);
        case cipher_chacha20poly1305:
            return cipher_chacha20poly1305_decrypt((const char *)data, (char *)output, len);
        default:
            return -1;
    }
}
